...
19. Test the HTTP service. In the web shell type (should be double dash in curl command before head as pictured in example output):
...
32. Test HTTP again (should be double dash in curl command before head as pictured in example output).  You should still receive a **200 OK** response because we allowed only HTTP in the firewall policy:
