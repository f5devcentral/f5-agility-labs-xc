...
#. Open another tab in your browser, navigate to the newly configured Load Balancer configuration: **http://<namespace>-lb.lab-sec.f5demos.com**, to confirm it is still functional.

#. Using some of the sample attacks below, add the URI path & variables to your application to generate security event data.

   .. code-block:: text

      http://<your-namespace>-lb.lab-sec.f5demos.com/q=%3cscript%3ealert%28%27xss%27%29%3c%2fscript%3e

      http://<your-namespace>-lb.lab-sec.f5demos.com/?4d4ad2dbdb=

      http://<your-namespace>-lb.lab-sec.f5demos.com/cart?search=aaa'><script>prompt('Please+enter+your+password');</script>
...
In either location let's setup our shell with a variable populated with you adjective-animal name and curl the following urls.
...
#. Returning to the F5 Distributed Cloud Console, use the left-hand navigation to navigate to **Web App & API Protection section** and click on **Security**

#. Scroll to the **Load Balancers** section of the page and click the link for your respective load balancer.

#. From the **Dashboard** view, using the horizontal navigation, click **Security Analytics**.

   .. note:: *If you lost your 1 Hour Filter*, be sure to adjust your time filter in the upper right corner to the last 1 hour.

#. Review the various charts and graphs on the Security Analytics dashboard.

#. Next, let's explore the Forensics Panel to get a more granular view of the security events. Using the horizontal navigation, click **Forensics Panel**.
...
#. In the Forensics Panel, you can see quick charts summarizing the security events over the selected time period. On the right side of each chart you can click the *pencil* icon to edit the chart settings.
...
#. From the Forensics Panel, select a WAF signature in the signatures.name chart by clicking on the bar in the chart. Once selected, click the **Apply Filter** button to filter the security events to only those that match the selected WAF signature.

|labsecurityforensicsfilter|

#. Review the filtered security events in the main table. You can click on any event to get more details about the specific security event.
